            ...
            @if ($tab === 'accounts')
                <table class="w-full text-sm">
                    <thead><tr class="border-b border-neutral-200 text-left text-[11px] uppercase tracking-wide text-neutral-500">
                        <th class="px-2 py-2.5">{{ t('common.member') }}</th>
                        <th class="px-2 py-2.5">{{ t('insurance.planName') }}</th>
                        <th class="px-2 py-2.5 text-right">{{ t('insurance.monthlyContribution') }}</th>
                        <th class="px-2 py-2.5 text-right">{{ t('insurance.coverageAmount') }}</th>
                        <th class="px-2 py-2.5 text-right">{{ t('insurance.totalContributions') }}</th>
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @forelse ($accounts as $a)
                            <tr>
                                <td class="px-2 py-2.5 text-[13px] font-medium">{{ $a->member?->full_name ?? '—' }}</td>
                                <td class="px-2 py-2.5 text-[13px]">{{ $a->plan_name }}</td>
                                <td class="px-2 py-2.5 text-right">{{ money($a->monthly_contribution) }}</td>
                                <td class="px-2 py-2.5 text-right">{{ money($a->coverage_amount) }}</td>
                                <td class="px-2 py-2.5 text-right">{{ money($a->total_contributed ?? 0) }}</td>
                                <td class="px-2 py-2.5"><x-status :status="$a->status" /></td>
                                <td class="px-2 py-2.5 text-right"><x-row-actions :delete="route('admin.insurance.accounts.destroy', $a)" /></td>
                            </tr>
                        @empty
